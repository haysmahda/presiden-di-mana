...
> **Status: done.** All six steps are complete. The site is live, bilingual, and runs
> itself — a robot re-reads the news **every 2 hours** and publishes any new location
> automatically. You don't have to do anything to keep it running.
...
You should see `Serving HTTP on 127.0.0.1 port 8000`. Leave this running.

> **Don't drop the `--bind 127.0.0.1`.** Without it the server is reachable from the whole
> internet and serves *everything* in this folder — including the hidden `.git` directory,
> which is enough for a stranger to download your entire project. With it, only your own
> machine can connect.
...
## Language, statistics and sharing

**Bilingual.** Every page has an **ID / EN** switch in the top-right. Indonesian is the
default; the choice is remembered in your browser. The legal disclaimer always appears in
both languages regardless of the setting, because it needs to.

**Statistics** on the Riwayat page: four headline figures, a ranked bar chart of the
most-visited regions, and a breakdown of days in Jakarta vs. elsewhere in Indonesia vs.
abroad. These are counted in distinct days rather than articles, so a busy news day
doesn't distort them.

**Share preview.** `og.png` is redrawn on every deploy with the current location baked in,
so pasting the link into WhatsApp or X shows *"Istana Merdeka"* rather than something
generic. It is generated by `scripts/og.js` from an SVG — you never edit an image by hand.
If that step ever fails the deploy continues anyway; the site is never blocked by it.
...
| `assets/i18n.js` | Every piece of text, in Indonesian and English |
| `assets/fonts/` | Instrument Serif (OFL licence), used to draw the share image |
| `scripts/update.js` | The news scanner — reads RSS, finds places, scores confidence |
| `scripts/og.js` | Draws `og.png`, the preview card shown when the link is shared |
...
- [x] **Step 4 — Automate.** GitHub Actions every 2 hours; live free on GitHub Pages.
- [x] **Step 5 — Polish.** Stats, share image, English toggle. ← *done*
